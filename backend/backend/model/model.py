...
def preprocess_data(df):
    ...
    required_columns = ["short_description", "tags", "genres", "reviews", "estimated_owners"]
    for col in required_columns:
        if col not in df.columns:
            raise KeyError(f"Missing required column: {col}")

    df["combined_features"] = (
        df["short_description"].fillna("") + " " + df["tags"].fillna("") + " " + df["genres"].fillna("")
    )
    df["score"] = 0
    df["combined_features"] = df["combined_features"].apply(lambda x: re.sub(r"[^a-zA-Z0-9\s.,!?\';:-]", "", x))
...
def get_tfidf_and_scaler(df, tfidf_path="./backend/model/tfidf.pkl", scaler_path="./backend/model/scaler.pkl"):
    if os.path.exists(tfidf_path) and os.path.exists(scaler_path):
        ...
        logging.info(f"TF-IDF saved to {tfidf_path}, Scaler saved to {scaler_path}")
...
def build_keras_model(input_dim):
    ...
    hidden_layer_1 = Dense(256, activation="relu", kernel_regularizer=l2(0.001))(input_layer)
    hidden_layer_2 = Dense(128, activation="relu", kernel_regularizer=l2(0.01))(hidden_layer_1)
...
def train_and_save_model(df, model_save_path="./backend/model/model.keras"):
    ...
    history = model.fit(
        X_train, y_train, epochs=5, batch_size=128, validation_data=(X_val, y_val), callbacks=[early_stopping]
    )
